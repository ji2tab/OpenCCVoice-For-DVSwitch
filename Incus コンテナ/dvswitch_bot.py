...
================================================================================
 ...
 バージョンごとの詳細な変更履歴（V1.60〜V1.94）は本ファイルには含めず、リポジトリ直下の
...
【使い方】
 ...
 Document Version: V1.94 (daemon, V1.93 + カーチャンク元が DMR ID 数字のとき
                          DMRIds.dat で解決／引けなければ応答スキップ)
...
# ============================================================
# 🔴 V1.94: DMR ID → コールサイン変換表（DMRIds.dat）
# ============================================================
# MMDVM_Bridge のログ行 "received network voice header from XXXX" の XXXX は、
# 送信元 DMR ID をローカルの ID→コールサイン表で引けたときはコールサイン、
# 引けなかったとき（late entry で LC デコードが間に合わない等）は DMR ID の
# 数字がそのまま入る。従来はこの数字をそのまま1文字ずつ読み上げていたため、
# 時々「フォー フォー ゼロ …」と ID を読む不具合になっていた（2026-07-08 実機）。
# 対策(A+B): カーチャンク元が全桁数字なら DMR ID とみなし、この DMRIds.dat で
# コールサインへ解決する。引ければそのコールサインで読み上げ、引けなければ
# 応答をスキップする（数字読みを根絶）。DMRIds.dat のフォーマットは
#   "<DMR_ID(数字)> <CALLSIGN> <名前など...>"（空白区切り）。
# 例) "4401378 JI2TAB Kazuhiko"
DMRIDS_PATH = "/var/lib/mmdvm/DMRIds.dat"
# 未知 ID（表に無い数字）を応答スキップにするか。
#   True  : スキップ（A+B。既定。数字読みを完全に防ぐ）
#   False : 数字のまま従来どおり読み上げる（V1.93 と同一挙動へ戻す退避用）
DMRID_SKIP_IF_UNKNOWN = True
...
# 🔴 V1.94: DMR ID → コールサイン変換表の状態（DMRIds.dat）。
# _load_dmrids() が読み込み、_resolve_callsign() が参照する。
# mtime を保持し、ファイルが更新されたら次回参照時に読み直す（再起動不要）。
_dmrids_map = {}          # { dmr_id(int): "CALLSIGN" }
_dmrids_mtime = None      # 読み込んだ時点の DMRIds.dat の mtime（None=未読）
_dmrids_lock = threading.Lock()
...
# ============================================================
# 🔴 V1.94: DMR ID → コールサイン変換（DMRIds.dat）
# ============================================================
def _load_dmrids(force=False):
    """DMRIds.dat を読み込み _dmrids_map を更新する。
    mtime が前回と同じなら（force でない限り）何もしない＝毎回の再読込を避ける。
    ファイルが無い/読めない場合は空の表のままにして False を返す（送出は止めない）。
    フォーマットは各行 "<DMR_ID> <CALLSIGN> <名前など...>"（空白区切り）。
    先頭が数字でない行・列不足の行は読み飛ばす。"""
    global _dmrids_map, _dmrids_mtime
    try:
        mtime = os.path.getmtime(DMRIDS_PATH)
    except OSError:
        # ファイルが無い等。表を空にし、mtime を None に戻す。
        with _dmrids_lock:
            if _dmrids_map or _dmrids_mtime is not None:
                _dmrids_map = {}
                _dmrids_mtime = None
        return False

    if (not force) and (mtime == _dmrids_mtime) and _dmrids_map:
        return True  # 変化なし・読み込み済み

    new_map = {}
    try:
        with open(DMRIDS_PATH, "r", encoding="utf-8", errors="ignore") as fh:
            for line in fh:
                parts = line.split()
                if len(parts) < 2:
                    continue
                idpart, callsign = parts[0], parts[1]
                if not idpart.isdigit():
                    continue
                try:
                    new_map[int(idpart)] = callsign.upper()
                except ValueError:
                    continue
    except OSError as e:
        logger.warning(_fmt("!!", "DMRIds", "read fail", str(e)))
        return False

    with _dmrids_lock:
        _dmrids_map = new_map
        _dmrids_mtime = mtime
    logger.info(_fmt("..", "DMRIds", "loaded", f"{len(new_map)} entries from {DMRIDS_PATH}"))
    return True


def _resolve_callsign(cs):
    """カーチャンク元文字列 cs を読み上げ用コールサインに解決する。
    返り値: (callsign or None, was_id)。
      - cs が英字を含む通常コールサイン → (cs, False)（従来どおり・変換不要）
      - cs が全桁数字（DMR ID）で表に有り → (CALLSIGN, True)
      - cs が全桁数字だが表に無い       → (None, True)（呼び出し側でスキップ判断）
    数字 ID のときは参照前に DMRIds.dat の更新を反映する（mtime 変化で再読込）。"""
    if not cs.isdigit():
        return cs, False   # 通常のコールサイン。V1.93 までと同一。

    # 全桁数字＝DMR ID とみなす。最新の表を反映してから引く。
    _load_dmrids()
    with _dmrids_lock:
        callsign = _dmrids_map.get(int(cs)) if cs.isdigit() else None
...
def _log_startup_info():
    logger.info("=" * 70)
    logger.info(f"DVSwitch Bot V1.94 (daemon, V1.93 + DMR ID→コールサイン解決) starting up (PID: {_PID})")
    ...
    # 🔴 V1.94: DMR ID → コールサイン変換（DMRIds.dat）の状態を表示
    if os.path.exists(DMRIDS_PATH):
        _unknown = "skip" if DMRID_SKIP_IF_UNKNOWN else "read digits (fallback)"
        logger.info(f"  DMR ID lookup     : ON  ({len(_dmrids_map)} entries / {DMRIDS_PATH} / "
                    f"unknown ID -> {_unknown})")
    else:
        logger.warning(f"  DMR ID lookup     : file NOT FOUND ({DMRIDS_PATH}) / "
                       f"数字 ID は {'スキップ' if DMRID_SKIP_IF_UNKNOWN else '数字読み'}")
...
def monitor_and_reply():
    ...
    _init_reply_cache()
    # 🔴 V1.94: DMR ID → コールサイン表（DMRIds.dat）を起動時に読み込む。
    # 以降は数字 ID を受けた時に mtime を見て自動で読み直す（再起動不要）。
    _load_dmrids(force=True)
    _log_startup_info()
    ...
    try:
        while not should_exit:
            ...
            if m_s:
                cs = m_s.group(1)
                # 🔴 V1.94: from の後ろが DMR ID の数字だった場合、DMRIds.dat で
                # コールサインへ解決する。ここで解決しておくことで、以降の先行生成
                # （キャッシュ鍵）・last_cs・カーチャンク読み上げのすべてが正しい
                # コールサインで一貫する。引けない数字 ID は（既定で）応答対象から
                # 除外し、last_cs も立てない＝後続の end/watchdog でも発火しない。
                resolved, was_id = _resolve_callsign(cs)
                if was_id:
                    if resolved is None:
                        if DMRID_SKIP_IF_UNKNOWN:
                            logger.info(f"receive:unknown DMR ID, skipped: {cs} "
                                        f"(not in {os.path.basename(DMRIDS_PATH)})")
                            continue
                        # スキップしない設定のときは数字のまま従来動作（退避用）。
                        resolved = cs
                    else:
                        logger.info(f"receive:DMR ID resolved: {cs} -> {resolved}")
                    cs = resolved
                if cs != MY_CALLSIGN:
