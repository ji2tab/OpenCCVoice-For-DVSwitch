...
- **定時アナウンス** — 1 時間に 1〜3 回、定時メッセージを交互送出
...
詳細は [構築導入設定 完全マニュアル](OpenCCVoice_構築導入設定_完全マニュアル.md) を参照してください。
以下は導入済み環境での要点のみです。
...
| ファイル | 役割 |
|---|---|
| [`dvswitch_bot.py`](dvswitch_bot.py) | 自動応答デーモン本体（JSON 設定を読むだけ。常駐向け） |
| [`bot_setup.py`](bot_setup.py) | Bot 設定ツール（`bot_config.json` を対話作成） |
| [`dvs_config.sh`](dvs_config.sh) | DVSwitch ini を TGIF 用に一括設定（自動バックアップ付き） |
| [`create_wav.sh`](create_wav.sh) | 固定 WAV を対話生成（自動バックアップ付き） |
| [`test_send.py`](test_send.py) | USRP 単発送信テスト |
