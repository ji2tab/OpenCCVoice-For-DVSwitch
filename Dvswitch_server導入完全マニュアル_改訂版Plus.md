# DVSwitch-Server 導入完全マニュアル（改訂版Plus）

## 改訂版 との差異について（改訂版Plus とは何か）

本ファイル（`Dvswitch_server導入完全マニュアル_改訂版.md`）は、同じく改訂版の内容を持つ **`Dvswitch server導入完全マニュアル 改訂版.md`**（スペース区切り）とほぼ同一の内容ですが、以下の点が異なります。

**改訂版 から 改訂版Plus への変更点：**

- **ファイル名をアンダースコア区切りに変更**：スペース区切り版（`Dvswitch server導入完全マニュアル 改訂版.md`）からアンダースコア区切り（`Dvswitch_server導入完全マニュアル_改訂版.md`）に変更。リポジトリ整理に伴うファイル配置の統一化対応です。
-  **第IV部（Platform 表示の修正）に Bookworm/Apache2 対応の注記を追加**：スペース区切り版（改訂版）では記載のなかった 🔷 Bookworm / Apache2 環境向けの注意事項（lighttpd ではなく apache2 を再起動する旨、STEP 6 への補足コマンド）を追記しました。

-  **技術内容・手順・その他の記述はスペース区切り版（改訂版）と完全に同一です。**
...
> 🔷 **環境差の注記（Bookworm / Apache2 の場合）**
> 本節は **bullseye + lighttpd** 環境を前提に記述している。新しめの構成、すなわち
> **Raspberry Pi OS (Bookworm) では Web サーバが Apache2** になっていることがある
> （DVSwitch Dashboard の DocumentRoot は `/usr/share/dvswitch`）。その場合は、
> 本節中の **`lighttpd` を `apache2` に読み替える**こと。特に STEP 6 の再起動は
> `sudo systemctl restart apache2` となる（STEP 6 にも注記あり）。
> 自分の環境がどちらかは次で確認できる:
> ```bash
> systemctl is-active lighttpd 2>/dev/null   # active なら lighttpd 構成
> systemctl is-active apache2  2>/dev/null   # active なら Apache2 構成
> ```
> なお、スクリプト本体（device-tree 優先で機種名を返すロジック）は Web サーバの
> 種類に依存しないため、どちらの環境でもそのまま使える。
...
> 🔷 **Bookworm / Apache2 の場合**は `lighttpd` ではなく Apache2 を再起動する:
> ```bash
> sudo systemctl restart apache2
> ```
> どちらが動いているか不明なときは `systemctl is-active lighttpd apache2` で確認する。
