...
"""
================================================================================
 voice_make.py (vmp) — OpenCCVoice for DVSwitch 定時音声 事前生成ツール（VVW版）
 ...
 【時報の正確さを守る仕組み】
  正時ちょうどに VOICEVOX 合成を始めたのでは間に合わない（合成に加え天気取得も
  伴い、天気込み長文ではさらに時間がかかる）。そこで2分前に「本来音声＋天気」を
 ...
 Document Version: V1.03vvm（VVW版・スロット先頭リード廃止〔素材WAVの助走へ移行〕＋
 　　　　　　　　　　　　　終わりの助走 SLOT_TAIL_SEC=1.0 を新設。JTW V1.03vmp と同等）
 　（V1.02vvm: JTW版 vmp V1.01vmp を土台に合成を VOICEVOX 化）
 　（V1.02vvm: スロット先頭に LEAD_BEFORE_INTRO_SEC=0.5s の無音を追加し
 　 時報/定時の頭欠けを解消。実測 2026-08-19・両ノード）
 　本ファイルは VVW版（VOICEVOX + 天気）。JTW版 voice_make.py（open_jtalk 合成）
 　との違いは合成部（_synth_middle）のみ: open_jtalk → vv_say.py（VOICEVOX）。
 　話者は vv_say.py が wav_source.json の voice を読むため、bot/create_wav と一致する。
 　スロット受け渡し・天気取得・地名・文生成・原子的差し替えは JTW版と同一。
 Last Updated: 2026-08-05
================================================================================
"""

__version__ = "V1.03vvm"
...
# 🔵 VVW: 合成は VOICEVOX（vv_say.py）。話者は vv_say が wav_source.json の
# voice を読むため、ここで話者パスを持つ必要はない。venv の python で叩く。
VV_SAY_PATH = "/opt/voicevox/vv_say.py"
...
# 🔴 V1.03vvm: 0.5 → 0.0（廃止・定数は巻き戻し用に残置）。語頭前の無音は素材WAV側
# （time_intro/001/002 の先頭 0.5s 助走＋20ms フェードイン）へ焼き込む方式に変更した。
# 素材が助走を持たない環境では 0.5 に戻すこと。JTW V1.03vmp と同等。
# 🔵 V1.03vvm: スロットの「終わりの助走」（秒）。結合後のスロット末尾を 20ms で
...
def _synth_middle(text, out48):
    """🔵 VVW: VOICEVOX（vv_say.py）で text を合成し WAV を out48 に書く。成功で True。
    vv_say.py は wav_source.json の voice（話者）を自分で読むため、bot や
    create_wav.sh と同じ話者で合成される（VVW の一貫性の要）。出力SRは vv_say に
    任せ（48k ハードコードはしない）、後段の _sox が 8k へ整える。"""
    try:
        r = subprocess.run(
            [VENV_PY, VV_SAY_PATH, text, out48],
            capture_output=True, text=True, timeout=60,
        )
        if r.returncode != 0:
            _log(f"vv_say.py 失敗 rc={r.returncode}: {(r.stderr or '').strip()[:200]}")
            return False
        return os.path.exists(out48)
    except FileNotFoundError as e:
        _log(f"vv_say.py / venv python 見つからず: {e}")
        return False
    except subprocess.TimeoutExpired:
        _log("vv_say.py がタイムアウト")
        return False
...
def _build_slot(base_kind, base_text, base_wav, weather_text, slot_path):
    ...
    try:
        ...
        # 🔴 V1.03vvm: 終端整形（フェードアウト 20ms ＋ 終わりの助走）
        if SLOT_TAIL_SEC > 0:
...
# ============================================================
def main():
    ap = argparse.ArgumentParser(description="OpenCCVoice 定時音声 事前生成 (vmp, VVW/VOICEVOX)")
