...
================================================================================
 ...
 — JJ2YYK デジピーター自動応答システム  V2.10jtw（JTW版）—
 ...
 【V2.10jtw の要点（V2.09jtw からの変更）】
  送出タイミングの実測総見直し（2026-08-20/21, ocv-uhf, RF+DroidStar ログ+45日1390サンプル）。
  頭切れの真因は3つ（素材WAVの助走欠如 / ノイズフロアの段差 / ストリーム先頭の SET_INFO）
  と判明し、いずれも解消。パディングは「音の間」から「プロトコル上の最小限」へ縮小した。
    1. PRE_PADDING_PACKETS 85→25（0.50s）。役目はヘッダ(VoiceLCHeader)保護に縮小。
       0 だとヘッダが中継で失われ受信側が幽霊ストリーム化する（実測）。
    2. POST_PADDING_PACKETS 85→6（0.12s）。切れ際の無音は WAV 側の終わりの助走へ移動。
    3. USRP_EOT_REPEAT 3→1（V1.87 実測に回帰）。0 は禁止・max(1,…) で保証。
    4. REPLY_TX_LEAD_DELAY_SEC 1.5→0.0。無送信待ちを廃し fixed_intro の助走 2.0s へ移動。
    5. REPLY_TAIL_SEC=1.0 新設。bot 生成音声にフェードアウト＋終わりの助走を付与。
    6. TX_METADATA_BEFORE_VOICE=False 新設。ストリーム先頭の SET_INFO は語頭を欠く（実測）。
    7. ASSERT_IDENTITY_ON_RX_END=False 新設。TA 主張は応答送出直前に一本化。
  【素材側の前提】fixed_intro.wav は先頭 2.0s、time_intro/001/002 は先頭 0.5s の無音助走
  ＋20ms フェードイン（ノイズフロア対策）を焼き込むこと。素材に助走が無いと頭切れが再発する。
  【巻き戻し】頭欠け→REPLY_TX_LEAD_DELAY_SEC を 0.5〜1.5 へ／TA が他局を引きずる→
  ASSERT_IDENTITY_ON_RX_END=True／切れ際異音→POST_PADDING_PACKETS を 20〜50 へ。

 【V2.09jtw の要点（V2.08jtw からの変更）】
  前後パディングの定数分離（リファクタのみ・既定値据え置き＝挙動不変）。
  従来 PRE_POST_PADDING_PACKETS 1個を前後で共有していたため、頭欠け対策で前を
  動かすと後（切れ際対策）まで必ず一緒に動いてしまい、独立に詰められなかった。
    1. PRE_PADDING_PACKETS（新設・既定85=1.7s）… 前パディング。頭欠け対策の実測確定値。
    2. POST_PADDING_PACKETS（新設・既定85=1.7s）… 後パディング。切れ際対策。長さは未検証。
    3. PRE_POST_PADDING_PACKETS は廃止（履歴記述を除きコードから参照を除去）。
    4. send_usrp_wav_with_padding() に post_packets 引数を追加。あわせて JTW にも
       pre_packets 引数を移植し、VVW と同一シグネチャに揃えた（従来 JTW は引数なし）。
  既定値は前後とも 85 のままなので、この版だけでは電波に出る音は1サンプルも変わらない。
  値の実測はこの版を入れた後に、前・後をそれぞれ独立に行う。

 【V2.08jtw の要点（V2.07jtw からの変更）】
  カーチャンク応答の頭欠け対策（実測確定のみ・機能変更なし）:
  REPLY_TX_LEAD_DELAY_SEC 0.5 → 1.5。ocv-uhf 実測（2026-08-19）で 0.5 では
  応答語頭が欠けたため、VVW で確定済みの 1.5 に統一。定時スロット側の頭欠けは
  voice_make.py V1.02vmp（LEAD_BEFORE_INTRO_SEC=0.5）で別途対処済み。
 ...
 【V2.05jtw の要点（V2.04jtw からの変更）】
  ...
  【巻き戻し】頭欠けが出たら PRE_PADDING_PACKETS を 85 → 90 と増やす方向で
...
【使い方】
 ...
 Document Version: V2.10jtw (daemon, 送出タイミング実測総見直し。前後パディング縮小
                          〔前0.5s/後0.12s〕・TX lead 廃止・終わりの助走・SET_INFO 整理。
                          素材 WAV への助走焼き込みが前提)
 　（V2.09jtw: 前後パディングを PRE_PADDING_PACKETS /
                          POST_PADDING_PACKETS へ分離〔既定値据え置き・挙動不変〕。
                          送出関数に post_packets 引数を追加し JTW/VVW のシグネチャを統一)
 　（V2.08jtw: カーチャンク応答頭欠け対策 REPLY_TX_LEAD 1.5s。旧: 送信切れ際のブブ/バリバリ対策。EOT 3発化＋
...
# 🔴 V2.10jtw: SET_INFO を音声ストリームの先頭（送出関数内）でも送るか。
# False（既定）にした理由: keyup=0 の SET_INFO の 20ms 後に音声ストリームが始まる
# 構成は、受信側で語頭が欠けることが RF 実測で確定（2026-08-20）。自局 TA の主張は
# 起動時と応答送出直前（_reply_executor 内・別ソケット）が担うため、ここは不要。
TX_METADATA_BEFORE_VOICE = False
# 🔴 V2.10jtw: 他局の受信終端のたびに SET_INFO を送るか（V1.90 の三重主張の (2)）。
# False（既定）にした理由: 応答しない受信（抑制中・Normal QSO）でも SET_INFO が飛び、
# ネットワーク側（DroidStar 等）に自局コールサインが単独で一瞬表示される。
# TA の維持は応答送出直前の主張で足りている。表示が他局を引きずる場合は True に戻す。
...
# ============================================================
# 🔴 V2.09jtw: 前後パディングの分離（値は据え置き＝挙動は V2.08jtw/V2.07vvw と同一）
# ============================================================
# 従来は PRE_POST_PADDING_PACKETS ただ1個を前後で共有していた。しかし 85 という値は
# 「前パディングの実測」だけで決まったもので、後パディングはその巻き添えである
# （旧コメント: 「前後で共有される値のため、後パディングも同じ 1.7s になる」）。
# 後パディング側で長さを検証した記録はコード・Changelog のどこにも無い。
# 頭切れを詰める作業で前だけを独立に動かせるようにするため、2定数へ分離する。
#
# 【前パディングの実測根拠】VV版 V1.98vv（2026-07-16/17, ocv-voicevox / 直 TGIF 接続）
#     前パディング 0.7s → 頭欠け（確実）
#     前パディング 1.5s → 頭欠け（3回中2回。V1.88 以来ずっとこの状態だった）
#     前パディング 1.7s → 頭欠けなし
# intro 頭欠けを決めているのはトーンの長さではなく「前パディング全体の長さ」だと
# 判明している（NOISE_LEAD_PACKETS の注記を参照。V1.88 のコメントは誤りだった）。
...
# 🔴 V2.10jtw: 85(1.7s) → 25(0.5s)。素材 WAV 側に助走（語頭前の無音）を焼き込んだため、
# 前パディングの役目は「TGIF ゲート対策の長い無音」から「VoiceLCHeader を通すための
# 非無音の先頭」に縮小した。0 にしてはならない（2026-08-21 実測: 前パディング 0 では
# 先頭トーンが消えて VoiceLCHeader が中継で失われ、受信側は送信開始を検出できず
# 終端信号だけで状態を反転させる＝数十秒の幽霊ストリーム化。25 で送信長が
# bot 実測と受信側記録で一致することを DroidStar ログで確認済み）。
#
# 【後パディング】目的は頭欠けではなく「送信の切れ際のブブ／バリバリ」対策である
# （V1.86 で完全ゼロ化 → V2.07jtw / V2.07vvw で末尾 NOISE_TAIL_PACKETS のみ微小トーン化）。
# 1.7s という長さ自体に実測の裏付けは無く、前パディングからの巻き添えで決まっていた。
# 短縮すれば送信占有時間がその分縮む（法令上も無駄な占有は短い方が良い）。ただし
# 切れ際の異音が復活しないか実機で耳確認してから決めること。未検証領域。
#     例) 40 → 0.8s、25 → 0.5s。NOISE_TAIL_PACKETS(=5) より小さくしないこと。
POST_PADDING_PACKETS = 6    # 6×20ms=0.12s（無音0.02+終端トーン0.10 / 終端フラッシュ）
# 🔴 V2.10jtw: 85(1.7s) → 6(0.12s)。切れ際の無音は vmp/_generate_hybrid が WAV 側に
# 付ける「終わりの助走」(1.0s) へ移した。残す 0.12s は DMR スーパーフレーム1個分の
# フラッシュで、末尾 NOISE_TAIL_PACKETS(5) のトーンが「長い完全無音の直後に閉じる」
# 状況を防ぐ（V2.07jtw の狙いを維持）。NOISE_TAIL_PACKETS より小さくしないこと。
...
USRP_EOT_REPEAT = 1
# 🔴 V2.10jtw: 3 → 1。V1.87 のキャプチャ実測（OCV のみ TerminatorLC が3個流れる・
# 通常局は1個・終端の多重が受信機の終端処理を乱す疑い）に従い単発へ戻す。
# 0 は禁止（送出ループの max(1,…) が下限を保証）: 2026-08-21 実測で、EOT を送らないと
# Analog_Bridge がストリームを閉じられず、受信側で数十秒の幽霊ストリームが観測された。
...
# 🔴 V2.10jtw: 1.5 → 0.0（廃止・定数は巻き戻し用に残置）。送出前の無送信待ちをやめ、
# 相当する時間は fixed_intro.wav 先頭の助走（2.0s・素材焼き込み）へ移した。
# キャリア断の直後に送信を開始しても応答の頭が欠けないことを RF 実測で確認済み
# （2026-08-21, ocv-uhf, JJ2ZAR 多数回）。頭欠けが出る環境では 0.5〜1.5 に戻すこと。
# 🔵 V2.10jtw: bot 生成音声（応答・起動・ナイト）の「終わりの助走」。_generate_hybrid が
# 結合後に 20ms のフェードアウト＋この秒数の無音を付ける。切れ際のバリバリ対策の本体は
# 終端の形（フラッシュ＋EOT 単発）で、これは音声とストリーム終端の間の緩衝。0 で無効。
REPLY_TAIL_SEC = 1.0
# 🔵 V2.08jtw: 0.5 → 1.5（実測 2026-08-19, ocv-uhf）。0.5 ではカーチャンク応答の
# 頭欠けが顕著だった（VVW/ずんだもんで確定した 1.5 と同値に統一）。JTW/VVW とも
# 1.5。話者・経路を変えたら必ず実機で頭欠けを再確認すること。
...
# （PRE_PADDING_PACKETS）と送出前ガード（REPLY_TX_LEAD_DELAY_SEC）に一本化する。
...
# 使用）だけを持つ「唯一の真実の源」として、各発火の約2分前に vmp を
# ワンショット起動（丙方式）し、「本体音声＋当地天気」の1本もの WAV を
# スロットへ事前生成させる。発火時はそのスロットを再生するだけ（合成ゼロ）。
#
# 設計の要点:
#   - 天気の ON/OFF・座標は bot_config.json（WEATHER_ENABLED 等）。bot はこの
#     フラグを「vmp に --weather を渡すか」の判断にだけ使い、取得も文生成もしない。
#   - スロット競合は構造的に起きない: 定時発火は最短15分間隔で排他的、合成は
#     数秒オーダー。生成が次発火に食い込むことは設計上ありえない。カーチャンク
#     応答は bot 側で別系統生成のため vmp と競合しない。→ ロック不要・固定名でよい。
#   - 割り切り: 発火時にスロットが無ければ鳴らさない（vmp 内で本体音声は死守
#     されるため、鳴らないのは vmp 自体の起動/本体合成が失敗した稀な場合のみ）。
#   - ナイトモード抑制・起動アナウンス・第30条 10分 ID・カーチャンク応答は
#     従来どおり bot 側で処理（vmp 管轄外・天気は付かない）。
VMP_PATH = f"{BOT_DIR}/bin/voice_make.py"
VMP_PREFETCH_LEAD_SEC = 120      # 発火の何秒前に vmp を起動して事前生成するか（=2分前）
VMP_SLOT_MAX_AGE_SEC = 900       # スロットWAVの鮮度上限（秒）。超えたら使わない（15分）
# スロットは「発火の分」ごとの固定名（/dev/shm）。競合しないため PID は付けない
# （bot 再起動後も同じ名を使うが、鮮度印 mtime で古いものは弾く）。
...
def _vmp_spawn(kind, minute, hour=None, base=None):
    ...
    kind: "time_signal" / "half_hour" / "message"
    hour: time_signal/half_hour で必須（読み上げる時）
    base: message で必須（本体WAVパス: 001.wav / cstm_001.wav 等）
    非ブロッキング（Popen で投げっぱなし）。2分前なので発火まで十分間に合う。
    stderr は bot の stderr に流れ、journal に vmp ログとして残る。"""
    slot = _slot_path(minute)
    cmd = [sys.executable, VMP_PATH, "--kind", kind, "--slot", slot]
    if hour is not None:
        cmd += ["--hour", str(hour)]
    if base is not None:
        cmd += ["--base", base]
    ...
    try:
        ...
        logger.info(_fmt("..", "vmp spawn", f":{minute:02d}",
                         f"{kind}"
...
def _slot_ready(minute):
    """発火時、minute のスロットが有効（存在＋鮮度内）なら パスを返す。
    無効なら None（bot は割り切りで鳴らさない）。使用後は呼び出し側が破棄。"""
    slot = _slot_path(minute)
    try:
        if os.path.exists(slot) and (time.time() - os.path.getmtime(slot)) <= VMP_SLOT_MAX_AGE_SEC:
            return slot
    except OSError:
        pass
    return None
...
def send_usrp_wav_with_padding(wav_path, pre_packets=PRE_PADDING_PACKETS,
    ...
    🔴 V2.09jtw: 前後パディングは PRE_PADDING_PACKETS / POST_PADDING_PACKETS の
    2定数に分離した（既定はどちらも 85 × 20ms = 1.7 秒で従来と同一）。
    pre_packets / post_packets 引数で呼び出し単位に上書きできる。"""
    sock = None
    wf = None
    seq = 0
    try:
        ...
        if sock:
            ...
            try:
                # 下限1は撤去禁止（8/21 実測: 0 だと AB がストリームを閉じられず幽霊化）
...
# ============================================================
# 🔵 V2.03jtw: スロット再生（送信時 TX_GAIN 制御）
# ============================================================
# vmp が作ったスロット WAV（素の音量）を送出する。TX_GAIN は「送信の属性」と
# して、生成側（vmp）ではなく送出の瞬間に bot がかける。こうすると (1) 音量変更が
# スロット作り直しなしで即反映され、(2) カーチャンク応答（bot 生成）と定時系
# （vmp 生成）で音量制御のかかり方が「送出時に一元」で揃う。
def _send_with_gain(wav_path):
    """wav_path を送出する。TX_GAIN が等倍以外なら vol をかけた一時ファイルを
    作ってから送る（fixed_file の従来手法と同一）。gain 適用に失敗しても素の
    ファイルで送出は続ける（無音化・送出停止を避ける）。"""
    play_path = wav_path
    if TX_GAIN is not None and TX_GAIN != 1.0:
        try:
            subprocess.run(["sox", wav_path, TEMP_FINAL, "vol", f"{TX_GAIN}"], check=True)
            play_path = TEMP_FINAL
        except (subprocess.CalledProcessError, FileNotFoundError) as e:
            logger.error(_fmt("!!", "Gain failed", os.path.basename(wav_path), str(e)))
            play_path = wav_path
...
def _all_trigger_minutes():
    """この設定で音声が出る全ての「分」を、種別つきで返す。
    返り値: dict {minute: (kind, hourly)} 。kind は "time_signal"/"half_hour"/
    "message"。message は 001/002 交互のため、発火順を保つよう昇順で扱う。
    - time_signal(:00) と half_hour(:30) は TIME_SIGNAL_MODE による。
    - message の分は _get_trigger_minutes()（時刻案内が占有する分は除かれている）。
    """
    out = {}
    if TIME_SIGNAL_MODE >= 1:
        out[0] = ("time_signal", None)
    if TIME_SIGNAL_MODE == 2:
        out[30] = ("half_hour", None)
    for mm in _get_trigger_minutes():
        out[mm] = ("message", None)
    return out


def _msg_base_for(msg_index):
    """001/002 交互の本体WAVパスを返す（cstm 解決込み）。号令時に bot が決めて
    vmp に渡す（cstm 判定は bot の一元管理・vmp は知らない）。"""
    idx = msg_index % len(MSG_FILES)
    use_cstm = USE_CSTM_001 if idx == 0 else USE_CSTM_002
    return _resolve_wav(use_cstm, CSTM_MSG_FILES[idx], MSG_FILES[idx],
                        os.path.basename(MSG_FILES[idx]))


def _night_suppressed_for(kind, hour):
    """種別ごとのナイト抑制判定。time_signal は N1 時のみ出す（突入告知）。
    half_hour / message は N1 時台から抑制。"""
    if kind == "time_signal":
        return _is_night_suppressed(hour)
    return _is_night_suppressed_message(hour)


def _announcement_scheduler():
    """🔵 V2.03jtw: 各発火の「約2分前」に vmp を号令してスロットを事前生成させ、
    発火時刻の直前（lead）にスロットを再生する2フェーズ構成。

    - スケジュール判定（何分に何を鳴らすか・ナイト抑制・001/002 交互・cstm）は
      すべてこの scheduler が持つ「唯一の真実の源」。vmp はスケジュールを知らない。
    - 号令フェーズ: 各発火分の VMP_PREFETCH_LEAD_SEC 秒前に一度だけ vmp を起動。
      抑制対象の分は号令しない（＝スロットが作られず、発火時も鳴らない）。
    - 発火フェーズ: 発火分の TIME_SIGNAL_LEAD_SEC 秒前に、スロットを再生。
    """
    spawned_key = None        # 直近に号令した (date, hour, minute)
    fired_key = None          # 直近に発火した (date, hour, minute)
    msg_index = 0             # 001/002 交互カウンタ（message を号令するたびに進める）

    logger.info(_fmt("..", "Scheduler", "started"))
    triggers = _all_trigger_minutes()
    logger.info(_fmt("..", "Scheduler", "triggers",
                     "分=" + ",".join(f"{mm:02d}({k})" for mm, (k, _) in sorted(triggers.items()))))

    while not should_exit:
        now = datetime.now()

        for minute, (kind, _) in triggers.items():
            cand = now.replace(minute=minute, second=0, microsecond=0)
            if cand <= now:
                cand += timedelta(hours=1)
            secs = (cand - now).total_seconds()
            hour = cand.hour
            key = (cand.date(), cand.hour, minute)

            # ---- 号令フェーズ（約2分前に1回）----
            # lead より前、かつ prefetch 窓に入ったら vmp を起動。
            if (TIME_SIGNAL_LEAD_SEC < secs <= VMP_PREFETCH_LEAD_SEC
                    and spawned_key != key):
                spawned_key = key
                if _night_suppressed_for(kind, hour):
                    logger.info(_fmt("..", "NightSkip", f"{hour:02d}:{minute:02d}",
                                     f"{kind} suppressed (no vmp spawn)"))
                else:
                    if kind == "message":
                        base = _msg_base_for(msg_index)
                        _vmp_spawn("message", minute, base=base)
                        msg_index += 1
                    elif kind == "time_signal":
                        _vmp_spawn("time_signal", minute, hour=hour)
                    else:  # half_hour
                        _vmp_spawn("half_hour", minute, hour=hour)

            # ---- 発火フェーズ（lead 秒前に1回）----
            if 0 < secs <= TIME_SIGNAL_LEAD_SEC and fired_key != key:
                fired_key = key
                if _night_suppressed_for(kind, hour):
                    # 号令もしていないので何もしない（ログは号令フェーズで既出）
                    pass
                else:
                    label = f"{hour:02d}:{minute:02d}"
                    logger.info(_fmt("..", "Trigger", label, f"{kind} (lead {TIME_SIGNAL_LEAD_SEC}s)"))
                    # 発火ワーカーには「分」を渡してスロットを再生させる。
                    _start_worker(kind if kind != "half_hour" else "half_hour_signal",
                                  minute, extra=label)
    ...
    try:
        if mode == "kerchunk":
            ...
            if path:
                ...
                # 🔴 V2.10jtw: TA 主張は「送出の直前」に一本化。別ソケットで SET_INFO を
        ...
        elif mode in ("time_signal", "half_hour_signal", "message"):
            # 🔵 V2.03jtw: 定時系はすべて「vmp が 2 分前に作ったスロットを再生」に
            # 統一した。val は発火の「分」(minute)。extra は表示用ラベル。
            # スロット有 → 送信時 TX_GAIN をかけて送出（使用後に破棄）。
            # スロット無/古い → 割り切りで鳴らさない（vmp 内で本体音声は死守される
            # ため、鳴らないのは vmp 起動失敗・本体合成失敗の稀な場合のみ）。
            minute = val
            label = extra or f":{minute:02d}"
            slot = _slot_ready(minute)
            if slot:
                logger.info(_fmt("TX", "Sending", label, f"{mode} (slot)"))
                _send_with_gain(slot)
                _safe_remove(slot)
                elapsed = time.monotonic() - started_at
                logger.info(_fmt("TX", "Slot", label, f"{elapsed:.1f}s"))
                # 時報が N1 時ちょうどならナイトモード突入告知（従来どおり bot 生成）
                if mode == "time_signal" and NIGHT_MODE_ENABLED:
                    try:
                        hour_now = datetime.now().hour
                    except Exception:
                        hour_now = -1
                    if hour_now == NIGHT_START_HOUR:
                        _send_night_mode_announcement()
            else:
                logger.warning(_fmt("!!", "Slot missing", label,
                                    f"{mode}: vmp 未生成/期限切れのため送出せず（割り切り）"))
...
def _generate_hybrid(intro, middle_text, outro, out_path=TEMP_FINAL, head_silence=0.0):
    ...
    with _gen_lock:
        try:
            ...
            # 🔴 V2.10jtw: 終端を 20ms でフェードアウトし、REPLY_TAIL_SEC の無音を付ける
...
def _log_startup_info():
    ...
    if WEATHER_ENABLED:
        ...
        logger.info(f"  Weather readout   : OFF (定時音声は天気なし)")
...
def monitor_and_reply():
    ...
    # 🔵 V2.03jtw: 天気の事前取得・事前生成は scheduler が発火2分前に voice_make.py
    # （vmp）をワンショット起動して行う（専用スレッドは持たない）。
