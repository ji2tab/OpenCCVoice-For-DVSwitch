...
> bot 本体が `ocv_dvs_jtw/` の `dvswitch_bot.py`（V2.00jtw〜）となり、毎正時の時報に続けて
> 当地の天気（Open-Meteo・API キー不要）を読み上げる機能が加わります（第17章 17-2 参照）。
> それ以外の構築手順は JT版と共通です。
>
> **対象構成:**
...
#### 天気読み上げ（`WEATHER_*`）— V2.00jtw〜（JTW版 / 設定ツール `bot_setup.py` は V2.02jtw）

毎正時の時報に続けて、Open-Meteo API（キー不要・追加インストール不要）から取得した当地の天気を読み上げる。

**動作:** 対象正時の約2分前に天気コード＋気温を取得し、「こちらは、〜。○時です。**続いて当地の天気は、晴れ、気温は25度です**」をイントロ込みの1本もの WAV として**事前生成**する。正時は完成 WAV を再生するだけなので、Open JTalk の合成時間（Pi で1.5秒超）による時報の遅れが出ない。

**三段フォールバック**（時報は絶対に落とさない）: ①事前生成 WAV → ②取得済みテキストからその場合成 → ③従来の「○時です」のみ。取得失敗時は天気だけ黙って省略され、時報は定刻に流れる。
...
| `WEATHER_HOURS` | 天気を読む正時の配列（例 `[7, 12, 18]`） | 省略時は全時刻。対話では `7,12,18` のようにカンマ区切り、`*` で全時刻 |

> 🔴 **制約:** `TIME_SIGNAL_MODE=0`（時刻案内なし）との併用は「天気を読む時報が存在しない」矛盾設定のため、本体が**起動を拒否**する。天気を使う場合はモード 1 または 2 にすること（天気が付くのは :00 の時報のみ。:30 の 30 分案内には付かない）。
> ナイトモードの抑制時刻は取得もスキップされる。

> ⚠️ **設定は必ず `bot_setup.py`（JTW版）で行うこと。** 旧版（JT版）の `bot_setup.py` やダッシュボード（現行 app.py）で設定を保存すると、`WEATHER_*` キーが消えて天気機能が黙って OFF に戻る。

動作確認はログで行う（`journalctl -u dvswitch-bot -f`）。対象正時の約2分前に `Weather ... 取得OK` → `Weather prebuilt ... 事前生成OK`、正時に `TimeSignal ... (prebuilt)` と出れば設計どおり。
...
| 時報に天気が付かない | 取得失敗（回線・Open-Meteo 側）or 対象時刻外 | 仕様どおり天気だけ省略し時報は定刻。ログに `Weather ... 取得失敗` があれば回線確認。`WEATHER_HOURS` に対象時刻が入っているか確認（第17章） |
