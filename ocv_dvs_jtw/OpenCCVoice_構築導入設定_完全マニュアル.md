# OpenCCVoice for DVSwitch 構築・導入・設定 完全マニュアル（JTW版）

**自動音声応答（デジピーター）システム — Bookworm + DVSwitch-Server 版＋時報への当地天気読み上げ**
...
>
> **JTW版について:** 本書は JT版マニュアルを母体とした **JTW版（JT版＋天気読み上げ）** です。
> bot 本体が `ocv_dvs_jtw/` の `dvswitch_bot.py`（V2.03jtw〜）となり、定時音声（時報・30分・
> 001/002）の後ろに当地の天気（Open-Meteo・API キー不要）を付ける機能が加わります。天気の
> 取得・合成は独立プログラム `voice_make.py`（vmp）が担い、bot は再生に専念します（第17章
> 17-2 参照）。それ以外の構築手順は JT版と共通です。
>
> **対象構成:**
...
BASE=https://raw.githubusercontent.com/ji2tab/OpenCCVoice-For-DVSwitch/main/ocv_dvs_jtw
...
#### 天気読み上げ（`WEATHER_*`）— V2.03jtw〜（JTW版 / bot・設定ツール・voice_make.py）

毎正時の時報に続けて、Open-Meteo API（キー不要・追加インストール不要）から取得した当地の天気を読み上げる。

**対象:** 時報（:00）・30分案内（:30）・定時メッセージ（001/002）の**すべての定時音声**の後ろに天気を付ける（カスタム 001/002 使用時はカスタム音声の後ろに付く）。

**動作（bot と voice_make.py の分離）:** 各定時音声の発火の約2分前に、bot が独立プログラム `voice_make.py`（vmp）をワンショット起動する。vmp は天気コード＋気温を取得し、「○時です。**○○の天気は、晴れ、気温は25度です**」のように本体音声＋天気を1本の WAV（スロット）に焼く。発火時、bot はこのスロットを再生するだけ（合成ゼロ＝遅延ゼロ）。地名（○○）は `wav_source.json` の `location`（`create_wav.sh` で設定するイントロの地名）を使う。

**フォールバック**（時報を絶対に落とさない）: 天気の取得・合成に失敗しても、vmp は本体音声（「○時です」等）だけはスロットに必ず書く。回線断や Open-Meteo 障害でも定時音声は定刻に流れ、天気だけ黙って省略される。発火時にスロットが無い（vmp 起動失敗等の稀な場合）ときは、その回は鳴らさない。

**音量（TX_GAIN）:** vmp は素の音量でスロットを作り、bot が送出の瞬間に TX_GAIN をかける（送信時制御）。音量を変えてもスロットの作り直しは不要で、カーチャンク応答と音量制御が一貫する。

`bot_setup.py` の対話（「--- 天気読み上げ（JTW版）---」セクション）で次の任意キーを設定する。

| キー | 意味 | 備考 |
|---|---|---|
| `WEATHER_ENABLED` | 天気読み上げの有効/無効 | 既定 `false`（未設定なら完全に従来動作） |
| `WEATHER_LATITUDE` | 取得座標の緯度（例 35.2167） | 有効時は必須。未設定なら `MMDVM_Bridge.ini` の `[Info]` Latitude/Longitude が初期値として提示される（配布既定値は除外）。Google マップで自局位置を右クリックしても確認できる |
| `WEATHER_LONGITUDE` | 取得座標の経度（例 137.0333） | 同上 |

> 天気文の地名は `wav_source.json` の `location`（`create_wav.sh` でイントロに設定する地名）を使う。地名を変えるときは `create_wav.sh` で地名を設定し直すと、イントロと天気の両方が一度に変わる。読みが不自然な地名は `create_wav.sh` の読み調整で対応できる。

> 🔴 **制約:** 時報も定時メッセージも無い設定（`TIME_SIGNAL_MODE=0` かつ `ANNOUNCE_FREQ=0`）との併用は「天気を付ける音声が存在しない」矛盾のため、本体が**起動を拒否**する。天気を使う場合はどちらかを有効にすること。
> ナイトモード中の定時音声は bot が号令しないため、天気の取得・生成もスキップされる（カーチャンク応答は従来どおり動作）。

> ⚠️ **設定は必ず `bot_setup.py`（JTW版 V2.03jtw）で行うこと。** 旧版の `bot_setup.py` やダッシュボード（現行 app.py）で設定を保存すると、`WEATHER_*` キーが消えて天気機能が黙って OFF に戻る。
>
> ⚠️ **`voice_make.py` の配置を忘れないこと。** bot と同じ `/opt/dvswitch_bot/bin/` に `voice_make.py` を置く。無いと bot 起動時に警告が出て、天気付き定時音声が生成されない（本体音声も生成されないため、その定時音声は鳴らない）。

動作確認はログで行う（`journalctl -u dvswitch-bot -f`）。各定時音声の約2分前に `vmp spawn :NN ...` と `[vmp] 天気取得OK ... → 「○○の天気は…」`、発火時に `Slot :NN ...` と出れば設計どおり。vmp を単体で試すには次を実行する（発火を待たず、その場でスロットを1本作れる）:
> ```
> python3 /opt/dvswitch_bot/bin/voice_make.py --kind time_signal --hour 12 --slot /dev/shm/test.wav --weather
> soxi /dev/shm/test.wav   # 地名入りの天気付き時報が1本できる
> ```
...
[Unit]
Description=DVSwitch Bot (OpenCCVoice JTW, daemon)
...
天気読み上げ（有効/無効・座標・対象時刻）の変更もこの手順で行う（JTW版 `bot_setup.py` が対応。ダッシュボードは現時点で `WEATHER_*` 非対応のため使わない — 第17章の注意参照）。
...
| Bot 設定 | `bot_config.json` の項目をフォームで編集・保存（送出音量 `TX_GAIN` を含む）。⚠️ **JTW版の `WEATHER_*` キーには未対応。ここで保存すると天気設定が消えるため、天気を使うノードの設定変更は `bot_setup.py` で行うこと** |
...
| 音声がプツプツ途切れる・歪む | UDP ドリフト | bot が `time.monotonic()` ベースの絶対時刻同期か確認 |
...
| 定時音声に天気が付かない | 取得失敗（回線・Open-Meteo 側） | 仕様どおり天気だけ省略し定時音声は定刻。ログに `[vmp] 天気取得失敗` があれば回線を確認 |
| 定時音声が鳴らない（時報も含め） | `voice_make.py` 未配置 or vmp 起動失敗 | `/opt/dvswitch_bot/bin/voice_make.py` の有無を確認。bot 起動ログに `[vmp] ... が見つかりません` が出ていないか確認 |
| 起動時に `WEATHER_ENABLED=true は TIME_SIGNAL_MODE=0 と併用できません` | 矛盾設定 | `TIME_SIGNAL_MODE` を 1 か 2 に変更、または天気を無効化（第17章） |
| 天気設定が消えて OFF に戻った | 旧版 `bot_setup.py` やダッシュボードで設定を保存した | JTW版 `bot_setup.py` で再設定。以後の設定変更は JTW版 `bot_setup.py` のみ使用（第17章） |
