# DVSwitch 自動音声応答システム 完全マニュアル V2.3Plus

## V2.3 との差異について（V2.3Plus とは何か）

本ファイル（`DVSwitch_自動音声応答システム_完全マニュアル_V2_0.md`）は、同じく V2.3 の内容を持つ **`DVSwitch 自動音声応答システム 完全マニュアル V2.0.md`**（スペース区切り）とほぼ同一の内容ですが、以下の点が異なります。

**V2.3 から V2.3Plus への変更点：**

- **ファイル名をアンダースコア区切りに変更**：スペース区切り版（`DVSwitch 自動音声応答システム 完全マニュアル V2.0.md`）からアンダースコア区切り（`DVSwitch_自動音声応答システム_完全マニュアル_V2_0.md`）に変更。リポジトリ整理に伴うファイル配置の統一化対応です。
-  **冒頭に「旧版警告ブロック」を追加**：本書が旧環境（Pi-Star/WPSD + dvswitch_bot158.py V1.58）の記録であることを明示する ⚠️【重要】ブロックを冒頭に追加しました。現行システム（Bookworm + `dvswitch_bot.py` + `/bin/` 配置）との違いを読者が即座に把握できるようにするためです。スペース区切り版（V2.3）にはこの警告ブロックはありません。

-  **技術内容・手順・教訓はV2.3と完全に同一です。**
