...
**エディション構成（4エディション）:** 本システムには音声合成エンジン（Open JTalk / VOICEVOX）× 天気読み上げの有無で4エディションがある。**JT版（Open JTalk 版）** は `ocv_dvs_jt/`（Raspberry Pi ノード向け、上記の構成が標準）、**VV版（VOICEVOX 版）** は `ocv_dvs_vv/`（x86_64 Linux 向け）。この2つに定時音声への当地天気読み上げ（Open-Meteo API）を加えたものが **JTW版** `ocv_dvs_jtw/` と **VVW版** `ocv_dvs_vvw/` で、JT版・VV版は天気なしの安定版として維持され、天気系の改修は `w` 付きの系譜で行う。本仕様書の本文は JT版を基準に記述し、VV版固有の差分は各所の「VV版」注記および第8章の systemd 例で補足する（JTW版・VVW版の天気機能は本書の対象外。各ディレクトリの README を参照）。Web ダッシュボード（`dashboard/app.py`）は全エディションで共用。版番号のサフィックス `vv` は VOICEVOX 系、`w` 付き（`jtw` / `vvw`）は天気読み上げ搭載を示す命名規約。
