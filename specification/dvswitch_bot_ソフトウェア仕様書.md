...
14） [VV版差分（V1.99vv）](#14-vv版差分v199vv)
...
本章は **VV版（`ocv_dvs_vv/dvswitch_bot.py` V1.99vv）** に固有の差分のみを記述する。第1〜13章の本文（判定ロジック・第30条対応・USRP パケット・キャッシュ機構・ナイトモード・スレッド構成など）は JT版・VV版で共通であり、下記以外は本文どおりに動作する。
...
### 14-6. 送出タイミング定数の実測調整（V1.98vv）

VV版では、本文 §3・§8 が示す既定値（JT版 V1.95 基準）に対し、次の3定数を実測値で上書きしている。2026-07-16〜17 に ocv-voicevox（直 TGIF 接続）で実測したもので、**送出ロジック自体は変更していない**（定数3つと、実態と食い違っていたコメントの修正のみ）。

| 定数 | 本文（JT版）既定 | VV版 V1.98vv〜 | 目的 |
|---|---|---|---|
| `REPLY_TX_LEAD_DELAY_SEC` | 1.0 | **0.5** | キャッシュ命中時の送出前待機（受信の残処理待ち）。0/0.1 では頭欠け、0.5 で解消 |
| `PRE_POST_PADDING_PACKETS` | 75（1.5s） | **85（1.7s）** | 前後パディング。頭欠けは前パディング全体長で決まる（0.7s/1.5s で欠け、1.7s で解消）。前後共有のため後パディングも 1.7s |
| `NOISE_LEAD_PACKETS` | 65（1.3s） | **5（0.1s）** | 前パディング先頭のリード音長。トーン長は頭欠けに寄与せず、短くするとゲート開放後の「ブーン」も消える |

頭欠けの原因は独立した2つ ──「受信の残処理が空く前のキーアップ（→ `REPLY_TX_LEAD_DELAY_SEC`）」と「TGIF ゲートが開く前の intro 開始（→ `PRE_POST_PADDING_PACKETS`）」── で、両方の対策が必要。定時放送（001/002・時報）は直前受信が無いため頭欠けせず、これが前者ガードの根拠（傍証）になっている。なお `REPLY_TX_LEAD_DELAY_SEC` の旧値 2.5（V1.95a）は、キャッシュ OFF 時の生成時間 ≒ 2.4s に揃えた代理値だったが、V1.96 の合成高速化（実測 0.9s）で根拠が失効していた。

効果（実測 / No.7 話者・JI2TAB 応答）: カーチャンク受信終了→intro 開始 **4.02s→2.22s**（1.8s 短縮）、受信終了→送信終了 15.26s→13.9s、頭欠けは出る（V1.88 以降ずっと発生）→出ない。§11 のログ例「`TX lead <CS> 1.0s`」は本文（JT版）基準の値で、VV版では `0.5s` と出力される。
...
*dvswitch_bot.py ソフトウェア仕様書（JT版 V1.95 / VV版 V1.99vv）*  
*対象: /opt/dvswitch_bot/bin/dvswitch_bot.py（JT版 daemon, V1.60〜V1.95 の集大成）／ ocv_dvs_vv/dvswitch_bot.py（VV版 V1.99vv）*  
