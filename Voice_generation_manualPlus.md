...
```bash
echo "こちらは、ジェイジェイツーワイワイケー、尾張旭、DMR、デジピーターです。ティージーアイエフイチロクハチと、エックスエルエックスハチサンヨンモジュールゼットに インターネット接続しています。" | \
  open_jtalk \
    -x /var/lib/mecab/dic/open-jtalk/naist-jdic \
    -m /usr/share/hts-voice/mei/mei_normal.htsvoice \
    -ow /tmp/002_raw.wav
...
```bash
# bot を止める (systemd の場合。サービス名はハイフン dvswitch-bot)
sudo systemctl stop dvswitch-bot

# テスト送信(各ファイル)
python3 /opt/dvswitch_bot/bin/test_send.py /opt/dvswitch_bot/fixed_intro.wav
python3 /opt/dvswitch_bot/bin/test_send.py /opt/dvswitch_bot/fixed_outro.wav
python3 /opt/dvswitch_bot/bin/test_send.py /opt/dvswitch_bot/time_intro.wav
