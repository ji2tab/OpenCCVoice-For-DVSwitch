...
送出音量 `TX_GAIN` も `bot_setup.py`／ダッシュボードで設定できる（→ 5-6）。
...
> 💡 **設定方法:** `TX_GAIN` は `bot_config.json` の任意キーで、現在は
> `bot_setup.py`（対話）とダッシュボード（app.py）の双方で設定でき、保存し直しても
> 消えない。下記の手動 JSON 編集は、ツールを使わず直接調整したいとき向けの代替手段。
...
### 6-5. カスタム音声（cstm）に差し替える — V1.73〜

bot が送出する intro（名乗り）・定時メッセージ001・002 を、自前の WAV に**個別に**差し替えられる。標準ファイル（`fixed_intro.wav`/`001.wav`/`002.wav`）はそのまま残し、`/opt/dvswitch_bot/` 直下に `cstm_intro.wav`/`cstm_001.wav`/`cstm_002.wav`（**8000Hz・モノラル・16bit**）を置く。

有効化は2通り（どちらでも同じ結果）。

- **ダッシュボード:** `http://<ラズパイのIP>:8081/` →「変更モード」→「カスタム音声 — 差し替え」カードで使う項目にチェック →「保存」（自動でサービス再起動・反映）。
- **対話設定:** `sudo python3 /opt/dvswitch_bot/bin/bot_setup.py` で intro/001/002 を個別に選ぶ。

`intro` をカスタムにすると、それを使う3か所（カーチャンク応答・起動アナウンス・ナイトアナウンスの名乗り）すべてに効く。時報イントロ `time_intro.wav` は差し替え対象外。

> 💡 **安全策（フォールバック）:** カスタムを ON にしても対応ファイルが無ければ**自動的に標準で鳴る**ため、送出は止まらない。判定は送出のたびに行うので、後からファイルを置けば**再起動なしで**次回から反映される（ON/OFF 設定の変更時は再起動が必要）。

詳しい作成手順・つまずきポイントは `カスタム音声.md` を参照。
...
| TX_GAIN を変えても音量が変わらない | 設定変更後に再起動していない／値が範囲外 | `bot_setup.py` かダッシュボードで設定し再起動。範囲は 0.0超〜5.0以下。不正値は等倍にフォールバック（5-6） |
| カスタムにしたのに標準のまま | ファイル名・置き場所が違う | `ls /opt/dvswitch_bot/cstm_*.wav` で名前と場所を確認（6-5） |
| ログに「フォールバック」と出る | カスタム ON だがファイルが無い | ファイルを置けば再起動なしで反映（6-5） |
